...
function doSetAverageRating(locationId, AvgRating) {
    Loc.updateOne({ _id: locationId }, {
        $set: { rating: AvgRating }
    },
        function (err) {
            if (!err) {
                console.log("Average rating updated to " + AvgRating + " in location " + locationId)
            } else {
                console.log(err)
            }
        }
...
module.exports.reviewsUpdateOne = function (req, res, next) {
    if (!req.params.locationId || !req.params.reviewId) {
        sendJsResponse(res, 400, { message: "No locationId or reviewId in request" })
        return
    }
